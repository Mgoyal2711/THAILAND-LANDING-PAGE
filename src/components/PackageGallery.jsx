...
function PackageCard({
  ...
  const packageKey = detailsKey || category;
  const packageInfo = PACKAGE_DETAILS[packageKey];

  if (!packageInfo || !src) return null;

  return (
    <article className="group relative w-full overflow-hidden rounded-2xl border border-white/50 bg-white shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="absolute left-2 top-2 z-20">
        <div className="flex items-center gap-1.5 rounded-full border-2 border-white bg-gradient-to-r from-blue-900 via-blue-500 to-blue-400 px-3 py-[6px] shadow-[0_0_22px_rgba(255,90,0,0.95)]">
          <span className="h-2 w-2 rounded-full bg-blue-400"></span>
          <span className="text-[10px] font-black uppercase tracking-[1.5px] text-white">
            {badgeText}
          </span>
      ...
      <div className="p-5">
        ...
        <div className="mb-2 mt-1.5 inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-green-700">
          ALL INCLUSIVE
        </div>

        <h3 className="text-2xl font-bold text-navy">
          <span className="inline box-decoration-clone rounded-sm bg-accent px-1 py-1 leading-tight">
            {packageInfo.title}
          </span>
        </h3>
        ...
        <button
          type="button"
          onClick={() =>
            setExpanded(expanded === packageKey ? null : packageKey)
          }
          className="mt-4 w-full rounded-xl border border-red-500 py-3 font-semibold text-red-600 transition hover:bg-red-50"
        >
          {expanded === packageKey
        ...
        {expanded === packageKey && (
          <div className="mt-4 rounded-xl border bg-slate-50 p-4">
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              {packageInfo.details?.map((item, index) => (
                <li key={`${packageKey}-${index}`}>✓ {item}</li>
              ))}
            </ul>
          </div>
        )}
...
export default function PackageGallery({ onEnquire }) {
  ...
  const packages = PACKAGE_CATEGORIES.flatMap((category) =>
    (PACKAGE_IMAGES_BY_CATEGORY[category] || [])
      .map((item) => {
        ...
        return {
          category,
          src: item.src,
          detailsKey: item.detailsKey || category,
        };
      })
      .filter((item) => item.src && item.detailsKey)
  );
  ...
  return (
    <section className="px-4 pb-12 pt-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="package-container reveal">
          <div className="flex flex-col gap-6 md:hidden">
            {packages.map(({ category, src, detailsKey }, index) => (
              <div
                key={`${detailsKey}-${src}`}
                ref={(node) => {
                  if (node && !mobileCardRefs.current[category]) {
