export const PACKAGE_CATEGORIES = [
  '3N|4D',
  '4N|5D',
  '4N|5D',
  '5N|6D',
  '6N|7D',
  '7N|8D',
  '8N|9D',
];

const BASE = import.meta.env.BASE_URL;

export const PACKAGE_IMAGES_BY_CATEGORY = {
  '3N|4D': [`${BASE}optimized/3-nights/0-display.jpg`],
  '4N|5D': [`${BASE}optimized/4-nights/1-display.jpg`],
  '4N|5D': [`${BASE}optimized/4-nights/1.1.jpg`],
  '5N|6D': [`${BASE}optimized/5-nights/2-display.jpg`],
  '6N|7D': [`${BASE}optimized/6-nights/3-display.jpg`],
  '7N|8D': [`${BASE}optimized/7-nights/4-display.jpg`],
  '8N|9D': [`${BASE}optimized/8-nights/5-display.jpg`],
};

export const PACKAGE_DETAILS = {
  '3N|4D': {
  title: 'Simply Thailand',
  price: '₹20,000 / Person',

  details: [
    '🏨 2 Nights Stay in Pattaya',
    '🏨 1 Night Stay in Bangkok',

    '⭐ 4 Star Premium Hotel',

    '🍽️ Daily Breakfast',

    '✈️ Airport Meet & Greet',

    '🚕 Private Airport Transfers',
    '🚐 Private Intercity Transfers',

    '🎭 Alcazar Show - VIP Seat',

    '🏝️ Coral Island Tour by Speed Boat with Lunch',

    '🌃 Walking Street Visit',

    '🏙️ Bangkok City Tour with Gems Gallery',

    '🛍️ Shopping in Bangkok',

    '🧑‍💼 Guided Sightseeing Tour',

    '🛡️ Travel Insurance',

    '⏰ Early Check-In (Subject to Availability)',

    '📞 24x7 On-Ground Assistance',

    '💰 All Local Taxes & GST Included',
  ],
},

  '4N|5D': {
  title: 'Wonderful Thailand',
  price: '₹27,000 / Person',

  details: [
    '🏨 Glow Pattaya or Similar – 2 Nights Stay',
    '🏨 Bangkok Palace or Similar – 2 Nights Stay',

    '⭐ 4 Star Premium Accommodation',

    '🍽️ Daily Breakfast Included',

    '✈️ Meet & Greet Service at Airport',

    '🚖 Private Airport Transfers',
    '🚐 Private Intercity Transfers',

    '🎭 Alcazar Show – VIP Seat',

    '🏝️ Coral Island Tour by Speed Boat with Lunch',

    '🌃 Walking Street Experience',

    '🚤 Floating Market with Rolling Boat',

    '🏙️ Bangkok City Tour with Gems Gallery',

    '🛍️ Shopping Experience in Bangkok',

    '🦁 Safari World & Marine Park with Lunch',

    '🧑‍💼 Guided Sightseeing Tour',

    '📄 Thailand Arrival Card Assistance',

    '🛡️ Travel Insurance Included',

    '⏰ Flight Delay Coverage (Up to 2 Hours)',

    '🏨 Early Check-In (Subject to Availability)',

    '📞 24×7 On-Ground Assistance',

    '💰 All Local Thai Taxes & GST Included',
  ],
},

  '4N|5D': {
  title: 'Amazing Thailand',
  price: '₹35,500 / Person',

  details: [
    '🏨 Glow Ao Nang Krabi or Similar – 2 Nights Stay',
    '🏨 Sunset Beach Resort Phuket or Similar – 2 Nights Stay',

    '⭐ 4 Star Premium Accommodation',

    '🍽️ Daily Breakfast Included',

    '✈️ Meet & Greet Service at Airport',

    '🚖 Private Airport Transfers',
    '🚐 Private Intercity Transfers',

    '🏝️ Four Island Tour by Speed Boat with Lunch',

    '🛕 Phuket City Tour with Big Buddha',

    '🌃 Bangla Road & Patong Beach Visit',

    '🚤 Phi Phi Island Premium Tour by Speed Boat with Lunch',

    '🧑‍💼 Guided Sightseeing Tour',

    '📄 Thailand Arrival Card Assistance',

    '🛡️ Travel Insurance Included',

    '⏰ Flight Delay Coverage (Up to 2 Hours)',

    '🏨 Early Check-In (Subject to Availability)',

    '📞 24×7 On-Ground Assistance',

    '💰 All Local Thai Taxes & GST Included',
  ],
},

  '5N|6D': {
  title: 'Amazing Thailand',
  price: '₹35,500 / Person',

  details: [
    '🏨 Glow Ao Nang Krabi or Similar – 2 Nights Stay',
    '🏨 Sunset Beach Resort Phuket or Similar – 3 Nights Stay',

    '⭐ 4 Star Premium Accommodation',

    '🍽️ Daily Breakfast Included',

    '✈️ Meet & Greet Service at Airport',

    '🚖 Private Airport Transfers',
    '🚐 Private Intercity Transfers',

    '🏝️ Four Island Tour by Speed Boat with Lunch',

    '🛕 Phuket City Tour with Big Buddha',

    '🌃 Bangla Road & Patong Beach Visit',

    '🚤 Phi Phi Island Premium Tour by Speed Boat with Lunch',

    '🧑‍💼 Guided Sightseeing Tour',

    '📄 Thailand Arrival Card Assistance',

    '🛡️ Travel Insurance Included',

    '⏰ Flight Delay Coverage (Up to 2 Hours)',

    '🏨 Early Check-In (Subject to Availability)',

    '📞 24×7 On-Ground Assistance',

    '💰 All Local Thai Taxes & GST Included',
  ],
},
  '6N|7D': {
  title: 'Blissful Thailand',
  price: '₹45,500 / Person',

  details: [
    '🏨 Glow Ao Nang Krabi or Similar – 2 Nights Stay',
    '🏨 Sunset Beach Resort Phuket or Similar – 2 Nights Stay',
    '🏨 Bangkok Palace or Similar – 2 Nights Stay',

    '⭐ 4 Star Premium Accommodation',

    '🍽️ Daily Breakfast Included',

    '✈️ Meet & Greet Service at Airport',

    '🚖 Private Airport Transfers',
    '🚐 Private Intercity Transfers',

    '🏝️ Four Island Tour by Speed Boat with Lunch',

    '🛕 Phuket City Tour with Big Buddha',

    '🌃 Bangla Road & Patong Beach Visit',

    '🚤 Phi Phi Island Premium Tour by Speed Boat with Lunch',

    '🏙️ Bangkok City Tour with Gems Gallery',

    '🛍️ Shopping Experience in Bangkok',

    '🧑‍💼 Guided Sightseeing Tour',

    '📄 Thailand Arrival Card Assistance',

    '🛡️ Travel Insurance Included',

    '⏰ Flight Delay Coverage (Up to 2 Hours)',

    '🏨 Early Check-In (Subject to Availability)',

    '📞 24×7 On-Ground Assistance',

    '💰 All Local Thai Taxes & GST Included',
  ],
},

  '7N|8D': {
  title: 'Thailand Exclusive',
  price: '₹52,000 / Person',

  details: [
    '🏨 Glow Ao Nang Krabi or Similar – 2 Nights Stay',
    '🏨 Sunset Beach Resort Phuket or Similar – 2 Nights Stay',
    '🏨 Glow Pattaya or Similar – 2 Nights Stay',
    '🏨 Bangkok Palace or Similar – 1 Night Stay',

    '⭐ 4 Star Premium Accommodation',

    '💰 Per Person: ₹52,000',
    '👨 Extra Adult: ₹39,000',
    '👶 Extra Child: ₹34,000',

    '🍽️ Daily Breakfast Included',

    '✈️ Meet & Greet Service at Airport',

    '🚖 Private Airport Transfers',
    '🚐 Private Intercity Transfers',

    '🏝️ Four Island Tour by Speed Boat with Lunch',

    '🛕 Phuket City Tour with Big Buddha',

    '🌃 Bangla Road & Patong Beach Visit',

    '🚤 Phi Phi Island Premium Tour by Speed Boat with Lunch',

    '🎭 Alcazar Show – VIP Seat',

    '🏖️ Coral Island Tour by Speed Boat with Lunch',

    '🌆 Walking Street Experience',

    '🏙️ Bangkok City Tour with Gems Gallery',

    '🛍️ Shopping Experience in Bangkok',

    '🧑‍💼 Guided Sightseeing Tour',

    '📄 Thailand Arrival Card Assistance',

    '🛡️ Travel Insurance Included',

    '⏰ Flight Delay Coverage (Up to 2 Hours)',

    '🏨 Early Check-In (Subject to Availability)',

    '📞 24×7 On-Ground Assistance',

    '💰 All Local Thai Taxes & GST Included',
  ],
},

'8N|9D': {
  title: 'Majestic Thailand',
  price: '₹58,000 / Person',

  details: [
    '🏨 Glow Ao Nang Krabi or Similar – 2 Nights Stay',
    '🏨 Sunset Beach Resort Phuket or Similar – 2 Nights Stay',
    '🏨 Glow Pattaya or Similar – 2 Nights Stay',
    '🏨 Bangkok Palace or Similar – 2 Nights Stay',

    '⭐ 4 Star Premium Accommodation',

    '💰 Per Person: ₹58,000',
    '👨 Extra Adult: ₹44,000',
    '👶 Extra Child: ₹38,000',

    '🍽️ Daily Breakfast Included',

    '✈️ Meet & Greet Service at Airport',

    '🚖 Private Airport Transfers',
    '🚐 Private Intercity Transfers',

    '🏝️ Four Island Tour by Speed Boat with Lunch',

    '🛕 Phuket City Tour with Big Buddha',

    '🌃 Bangla Road & Patong Beach Visit',

    '🚤 Phi Phi Island Premium Tour by Speed Boat with Lunch',

    '🎭 Alcazar Show – VIP Seat',

    '🏖️ Coral Island Tour by Speed Boat with Lunch',

    '🌆 Walking Street Experience',

    '🏙️ Bangkok City Tour with Gems Gallery',

    '🛍️ Shopping Experience in Bangkok',

    '🦁 Safari World & Marine Park with Lunch',

    '🧑‍💼 Guided Sightseeing Tour',

    '📄 Thailand Arrival Card Assistance',

    '🛡️ Travel Insurance Included',

    '⏰ Flight Delay Coverage (Up to 2 Hours)',

    '🏨 Early Check-In (Subject to Availability)',

    '📞 24×7 On-Ground Assistance',

    '💰 All Local Thai Taxes & GST Included',
  ],
},
};